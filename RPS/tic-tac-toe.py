...
def str2tuple(string):  # Input: '(1,1)'
    ...
    return (int(string2list[1]), int(string2list[3]))  # Output: (1,1)
...
class Game():
    ...
    def switchMove(self):  # player-switch
        move = self.currentMove
        if move == 'blue':
            self.currentMove = 'red'
        elif move == 'red':
            self.currentMove = 'blue'
    ...
    def step(self, action_):
        # action = {'mark':'circle'/'cross', 'pos':(x,y)} # generate state
        x = action_['pos'][0]
        y = action_['pos'][1]
    ...
    def judgeEnd(self):
        ...
        if check_diag_1 == 3 or check_diag_2 == 3:
            ...
            self.winner = 'red'
            return True
        # check horizonal or vertical line
        state_T = self.state.T
        for i in range(3):
            check_row = sum(self.state[i])  # row
            check_col = sum(state_T[i])  # column
...
class Agent():
    ...
    def getEmptyPos(self, state_):  # return empty position
        action_space = []
    ...
    def epsilon_greedy(self, state_, currentMove):  # ε-greedy
        state = state_.copy() if currentMove == 'blue' else self.overTurn(state_)
...
class UserInterface():
    ...
    def render(self):
        ...
        # 显示分隔线
        for i in range(4):
            pygame.draw.line(self.window, self.DarkKhaki, (50, 50 + i * self.SIZE_L),
                             (50 + 3 * self.SIZE_L, 50 + i * self.SIZE_L), 8)
            pygame.draw.line(self.window, self.DarkKhaki, (50 + i * self.SIZE_L, 50 + 3 * self.SIZE_L),
                             (50 + i * self.SIZE_L, 50), 8)
        ...
        if self.isCheat:
            ...
            for i in range(4):
                pygame.draw.line(self.window, self.Black, (740, 180 + i * self.SIZE_M),
                                 (740 + 3 * self.SIZE_M, 180 + i * self.SIZE_M), 2)
                pygame.draw.line(self.window, self.Black, (740 + i * self.SIZE_M, 180 + 3 * self.SIZE_M),
                                 (740 + i * self.SIZE_M, 180), 2)
